...
def change_to_and(best_gate_dic, v_path, base, library_file):
    new_dic = best_gate_dic
    best_and = best_gate_dic['and'][0]
    base_command = f'./{base}'
    #not
    with open(f'{v_path}/and_buf.v', 'w') as f:
 ...
 {best_and} (a, 1, o);
    ...
    try:    
            command = [base_command, "-library", library_file, "-netlist", f'{v_path}/and_buf.v', "-output", 'owo.out']
            ...
            if os.path.exists('owo.out'):
                ...
                with open('owo.out', 'r') as f:
                    ...
                    if output_data.startswith("cost = "):
                        ...
                        if cost_value < best_gate_dic['buf'][1]:
                             new_dic['buf'] = ('change_and_buf', cost_value)
            else:
                print(f"Output file '{'owo.out'}' not found.")
    except subprocess.CalledProcessError as e:
        print(f"Error executing command for {f'{v_path}/and_buf.v'}: {e}")
