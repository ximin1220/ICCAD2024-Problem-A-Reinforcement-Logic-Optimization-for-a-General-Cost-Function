...
def change_to_nor(best_gate_dic, v_path, base, library_file):
    new_dic = best_gate_dic
    best_nor = best_gate_dic['nor'][0]
    base_command = f'./{base}'
    #not
    with open(f'{v_path}/nor_not.v', 'w') as f:
 ...
 {best_nor} (a, a, o);
    ...
    try:    
            command = [base_command, "-library", library_file, "-netlist", f'{v_path}/nor_not.v', "-output", 'owo.out']
                ...
                with open('owo.out', 'r') as f:
                    ...
                    if output_data.startswith("cost = "):
                        ...
                        if cost_value < best_gate_dic['not'][1]:
                             new_dic['not'] = ('change_nor_not', cost_value)
            else:
                print(f"Output file '{'owo.out'}' not found.")
    except subprocess.CalledProcessError as e:
        print(f"Error executing command for {f'{v_path}/nor_not.v'}: {e}")
