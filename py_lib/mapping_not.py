import re
# get names [and, not, nor]
def mapping_not(input_file, output_file, gate_names):
    s = set()
    ...
    with open(output_file, 'w', newline = '\n') as file:
        for line in lines:
            ...
            if not line.split() or line.split()[0] != 'assign':
                ...
                if len(line_split) == 4:
                    if line_split[3][0] == '~':
                        file.write(f'{gate_names[1]} ({line_split[3][1:-1]}, {line_split[1]});'+'\n')
                    else:
                    ...
                    if i1[0] == '~' and i2[0] == '~':
                        ...
                        if i1[0] == '~':
                            ...
                            if not nw in s:
                                s.add(nw)
                                file.write(f'{gate_names[1]} ({i1[1:]}, {nw});'+'\n')
    ...
    #                         file.write(f'wire {nw};\n')
                            if not nw in s:
                                s.add(nw)
                                file.write(f'{gate_names[1]} ({i2[1:-1]}, {nw});'+'\n')
                            new_i2 = nw
